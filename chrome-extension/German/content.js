...
function processSafeScoreChange(scoreChild, parentDiv, currentScore, sportEventName, scoringTeam = 'unknown') {
  ...
  try {
    ...
    logScoreChange({
      eventName: 'Goal',
      // Always set to "Goal" as requested
      score: safeScore,
      message: processedEventName,
      // Set the inner text of the <a> tag as message with bold team
      scoringTeam: scoringTeam,
      isScoreChange: true,
      isHtml: true,
      // Indicate that the message contains HTML
    ...
    if (scoreChild) {
      ...
      const observer = new MutationObserver((mutationsList, observer) => {
        ...
        try {
          ...
          const historyMatch = scoreHistory.find((item) => item.score === currentScore && now - item.time < 2000);
          if (historyMatch) {
          ...
          try {
            ...
            if (
              oldScoreParts.length === 2 &&
              newScoreParts.length === 2 &&
              !isNaN(oldScoreParts[0]) &&
              !isNaN(oldScoreParts[1]) &&
              !isNaN(newScoreParts[0]) &&
              !isNaN(newScoreParts[1])
            ) {
